...
def load_features(args):
    ...
    if os.path.exists(ckpt_file):
        ...
        if args.dataset == 'MNIST':
            ...
            cset = AttributeDataset(root='/home/ruizhe/ModelDebias/Baselines/Learning-Debiased-Disentangled-master/data/ColoredMNIST-Counterfactual', split='eval', transform = transforms['ColoredMNIST']['eval'])
            ...
            c_indices = (y_c.eq(3) + y_c.eq(8)).gt(0)
            #c_indices = (y_c.eq(3)).gt(0)
            X_train = X_train[train_indices]
            y_train = y_train[train_indices].eq(3).float()
            X_test = X_test[test_indices]
            y_test = y_test[test_indices].eq(3).float()
            X_c = X_c[c_indices]
            y_c = y_c[c_indices].eq(3).float()
            b_c = b_c[c_indices]
            
            c_indices2 = (b_c.eq(3)).gt(0)
            c_indices3 = (b_c.eq(8)).gt(0)
            X_c2 = X_c[c_indices2]
            y_c2 = y_c[c_indices2]
            X_c3 = X_c[c_indices3]
            y_c3 = y_c[c_indices3]
    ...
    return X_train, X_test, y_train, y_train_onehot, y_test, trainset, testset, X_c, y_c, X_c2, y_c2, X_c3, y_c3
